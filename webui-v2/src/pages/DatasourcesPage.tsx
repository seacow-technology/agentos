    ...
    try {
      ...
      if (data.ok && data.data && data.data.sources) {
        const transformedSources: DatasourceRow[] = data.data.sources.map((source: KnowledgeSource) => ({
          ...
          meta: [
            { key: 'type', label: 'Type', value: source.type },
            { key: 'status', label: 'Status', value: source.status },
            { key: 'chunks', label: 'Chunks', value: source.chunk_count.toLocaleString() },
            { key: 'created', label: 'Created', value: new Date(source.created_at).toLocaleDateString() },
          ],
