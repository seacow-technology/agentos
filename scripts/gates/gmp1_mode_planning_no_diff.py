#!/usr/bin/env python3
"""
Gate GMP1: Planning Mode No Diff (Static Assertion)

测试：planning mode 禁止 diff（配置级验证）
"""
...
def main():
    ...
    print("=" * 60)
    print("Gate GMP1: Planning Mode No Diff")
    print("=" * 60)
    
    # 执行断言
    internal_start = time.perf_counter()
    all_passed, assertions = assert_no_diff_mode("planning")
    ...
    json_path = write_gate_results(
        gate_id="gmp1_planning_no_diff",
        gate_name="Planning Mode No Diff (Static Assertion)",
    ...
    print("\n" + "=" * 60)
    print(f"{'✅ PASS' if all_passed else '❌ FAIL'}: Gate GMP1 (internal: {internal_duration:.2f}ms, process: {wall_duration:.2f}ms)")
