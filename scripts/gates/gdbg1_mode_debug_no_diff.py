#!/usr/bin/env python3
"""
Gate GDBG1: Debug Mode No Diff (Static Assertion)

测试：debug mode 禁止 diff（配置级验证）
"""
...
def main():
    ...
    print("=" * 60)
    print("Gate GDBG1: Debug Mode No Diff")
    print("=" * 60)
    
    # 执行断言
    internal_start = time.perf_counter()
    all_passed, assertions = assert_no_diff_mode("debug")
    ...
    json_path = write_gate_results(
        gate_id="gdbg1_debug_no_diff",
        gate_name="Debug Mode No Diff (Static Assertion)",
    ...
    print("\n" + "=" * 60)
    print(f"{'✅ PASS' if all_passed else '❌ FAIL'}: Gate GDBG1 (internal: {internal_duration:.2f}ms, process: {wall_duration:.2f}ms)")
