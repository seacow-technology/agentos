...
in AgentOS. All database access MUST go through this module.
...
Exceptions:
- agentos/core/db/writer.py: Can use sqlite3.connect() for write operations
...
def _get_db_path() -> str:
    ...
    Priority:
    1. AGENTOS_DB_PATH environment variable (backward compatibility)
        ...
        with _DB_PATH_LOCK:
            if _DB_PATH is None:
                ...
                if env_path:
                    ...
                    _DB_PATH = str(component_db_path("agentos").resolve())
...
def get_db() -> sqlite3.Connection:
    ...
    if not hasattr(_thread_local, "connection") or _thread_local.connection is None:
        ...
        DEBUG_TRACE_CLOSE = os.getenv("AGENTOS_DEBUG_DB_CLOSE", "false").lower() == "true"
        if DEBUG_TRACE_CLOSE:
            from agentos.core.db._connection_wrapper import DebugConnection
            conn = DebugConnection(conn)
            logger.warning("🔍 DB close tracing ENABLED (set AGENTOS_DEBUG_DB_CLOSE=false to disable)")
